package de.uni.tuebingen.sfs.java2;

public class History {
    public String searchTopic;
    public String target;
    public int match;
    public double frequency;

    public History(String searchTopic, String target, int match) {
        this.searchTopic = searchTopic;
        this.target = target;
        this.match = match;
    }

    public String toString() {
        String html = "<html>" +
                "<head>" +
                "<style>" +
                "body {font-family: Arial, sans-serif; margin: 0; padding: 15px; background-color: transparent;}" +
                ".container {max-width: 600px; margin: auto; padding: 15px; border-radius: 10px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); color: #9999BD;}" +
                "h1 {text-align: center; color: #fff; font-size: 20px;}" +
                "p {font-size: 14px; line-height: 1.6; color: #fff;}" +
                ".highlight {font-weight: bold; color: #000}" +
                "</style>" +
                "</head>" +
                "<body>" +
                "<div class='container'>" +
                "<h1>Search History</h1>" +
                "<p>You have searched for <span class='highlight'>" + searchTopic + "</span><br>" +
                "in <span class='highlight'>" + target + "</span>.</p>" +
                "<p>You got <span class='highlight'>" + match + "</span> matches <br>" +
                "</div>" +
                "</body>" +
                "</html>";
        return html;
    }

}
